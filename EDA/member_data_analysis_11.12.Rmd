...
Input Data:
- One cohort of members (e.g. 
members who joined in Jan 2017) (TBD)  
...
ggplot(member, aes(RENEW_IND, fill = autorenew_ind))+
  geom_bar()
```
...
```{r}
member$nbr_children = ifelse(
  member$nbr_children_desc==0, "0", ifelse(
    member$nbr_children_desc>0&member$nbr_children_desc<=3, "1-3","4+"
  )
)
member %>% 
  ggplot(aes(x=nbr_children, fill=renew_ind1))+
  geom_bar(position = "dodge")+
  facet_wrap(`)
```
member$nbr_children_desc = as.factor(member$nbr_children_desc)

member %>% 
  ggplot(aes(x=nbr_children_desc, y = marital_status_desc))+
  geom_tile()+
  scale_fill_gradient()
...
member %>% 
  ...
  ggplot(aes(x=hhh_age_desc, fill = renew_ind1))+
...
member %>% 
  ...
  ggplot(aes(x=MILES_TO_CLUB, color = RENEW_IND))+
  geom_density()+
  facet_wrap(~MEMBERSHIP_TYPE_DESC,2)+
  theme_bw()
```
...
```{r}
member %>% 
  ggplot(aes(x = marital_status_desc)) +
